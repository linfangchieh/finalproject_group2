...
class Page1(tk.Frame):
	...
	def creatWidgets(self):
		...
		self.blank_c2 = tk.Label(self, height = 1, width = 6, font = f4)
		#下拉選單-幣別1
		self.labelname1 = tk.Label(self, text = " 幣別 1： ", height = 1, width = 10, font = f2)
		self.droplist1 = tt.Combobox(self, width = 10, values = ["請選擇幣別1"," 美金 (USD) ", " 港幣 (HKD) ", " 英鎊 (GBP) ",
		...
		self.droplist2 = tt.Combobox(self, width = 10, values = ["請選擇幣別2"," 美金 (USD) ", " 港幣 (HKD) ", " 英鎊 (GBP) ",
		                                                    " 澳幣 (AUD) ", " 加拿大幣 (CAD) ", " 新加坡幣 (SGD) ",
		                                                    " 瑞士法郎 (CHF) ", " 日圓 (JPY) ", " 南非幣 (ZAR) ",
		                                                    " 瑞典幣 (SEK) ", " 紐元 (NZD) ", " 泰幣 (THB) ",
		                                                    " 菲國比索 (PHP) ", " 印尼幣 (IDR) ", " 歐元 (EUR) ",
		...
		self.cvsMain1.grid(row = 8, column = 0, rowspan = 20, columnspan = 10, sticky = tk.NE + tk.SW)
		self.cvsMain2.grid(row = 8, column = 5, rowspan = 20, columnspan = 10, sticky = tk.NE + tk.SW)
	...
	def makeLinechart(self, money, picturename):
		...
		py.legend(loc = 2, bbox_to_anchor=(1.05,1.0),borderaxespad = 0)
		py.xlabel("Date")
		py.ylabel("Cash Rate")
		py.xticks([])
	...
	def clickBtn(self):
		...
		if self.droplist1.get() != "請選擇幣別1":
			...
			self.imageMain1 = ImageTk.PhotoImage(file = self.picturename + ".png")
			self.image1 = self.cvsMain1.create_image(200, 200, image = self.imageMain1)
			os.system("del line.png")
		...
		if (self.droplist2.get() != "請選擇幣別2") and (self.droplist1.get() != self.droplist1.get()):
			if self.exit2 == 0:
				self.exit2 = 1
			elif self.exit2 == 1:
			...
			self.imageMain2 = ImageTk.PhotoImage(file = self.picturename + ".png")
			self.image2 = self.cvsMain2.create_image(200, 200, image = self.imageMain2)
			os.system("del line.png")
			# elif (self.droplist2.get() != "請選擇幣別2") and (self.droplist1.get() == self.droplist1.get()):

		else:
			if self.exit1 != 0:
				self.cvsMain1.delete(self.image2)
